...
class Dashboard:
    ...
    def verify_items_are_added_to_inventory(self, item_name_list):
        for item_name in item_name_list:
            expect(self.page.locator(".inventory_item").filter(has_text=item_name).get_by_role("button")).to_have_text("Remove")

    def verify_items_are_removed_from_cart(self, item_name_list):
        for item_name in item_name_list:
            expect(self.page.locator(".inventory_item").filter(has_text=item_name).get_by_role("button")).to_have_text("Add to cart")
